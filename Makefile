...
eb-create:
	@eb create ${ENV_NAME} --region=${AWS_DEFAULT_REGION} --cname="${ENV_NAME}" --elb-type application --tags Team=a-team,Project=mati --envvars \
		MP_JWT_VERIFY_ISSUER=${MP_JWT_VERIFY_ISSUER},MP_JWT_VERIFY_PUBLICKEY_LOCATION=${MP_JWT_VERIFY_PUBLICKEY_LOCATION},COGNITO_CLIENT_ID=${COGNITO_CLIENT_ID},COGNITO_CLIENT_SECRET=${COGNITO_CLIENT_SECRET},QUARKUS_DATASOURCE_USERNAME=${QUARKUS_DATASOURCE_USERNAME},QUARKUS_DATASOURCE_PASSWORD=${QUARKUS_DATASOURCE_PASSWORD},QUARKUS_DATASOURCE_JDBC_URL=${QUARKUS_DATASOURCE_JDBC_URL},NET=${NET},AGR_MATI_RELEASE=${RELEASE}
