...
eb-create:
	@eb create ${ENV_NAME} --region=${AWS_DEFAULT_REGION} --cname="${ENV_NAME}" --elb-type application --envvars \
		MP_JWT_VERIFY_ISSUER=${MP_JWT_VERIFY_ISSUER},MP_JWT_VERIFY_PUBLICKEY_LOCATION=${MP_JWT_VERIFY_PUBLICKEY_LOCATION},OKTA_CLIENT_ID=${OKTA_CLIENT_ID},OKTA_CLIENT_SECRET=${OKTA_CLIENT_SECRET},QUARKUS_DATASOURCE_USERNAME=${QUARKUS_DATASOURCE_USERNAME},QUARKUS_DATASOURCE_PASSWORD=${QUARKUS_DATASOURCE_PASSWORD},QUARKUS_DATASOURCE_JDBC_URL=${QUARKUS_DATASOURCE_JDBC_URL},NET=${NET},AGR_MATI_RELEASE=${RELEASE}
